...
public async Task<IActionResult> SaveFields(
    Guid id,
    List<SavePdfTemplateFieldRequest> request,
    CancellationToken ct)
{
    if (!await db.PdfTemplates.AnyAsync(x => x.Id == id, ct))
        return NotFound();

    if (request.Count > 250)
        return BadRequest(new
        {
            message = "La plantilla no puede contener más de 250 campos."
        });

    foreach (var x in request)
    {
        if (string.IsNullOrWhiteSpace(x.FieldKey)
            || x.XPercent is < 0 or > 100
            || x.YPercent is < 0 or > 100
            || x.WidthPercent <= 0
            || x.HeightPercent <= 0
            || x.XPercent + x.WidthPercent > 100.5
            || x.YPercent + x.HeightPercent > 100.5)
        {
            return BadRequest(new
            {
                message = $"Coordenadas inválidas para {x.Label}."
            });
        }
    }

    var existing = await db.PdfTemplateFields
        .Where(x => x.PdfTemplateId == id)
        .ToListAsync(ct);

    db.PdfTemplateFields.RemoveRange(existing);

    var fields = request.Select(x => new PdfTemplateField
    {
        PdfTemplateId = id,
        FieldKey = x.FieldKey,
        Label = x.Label,
        FieldType = x.FieldType,
        PageNumber = Math.Max(1, x.PageNumber),
        XPercent = x.XPercent,
        YPercent = x.YPercent,
        WidthPercent = x.WidthPercent,
        HeightPercent = x.HeightPercent,
        FontSize = x.FontSize,
        OccupantIndex = x.OccupantIndex
    }).ToList();
