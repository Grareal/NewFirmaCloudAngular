    ...
    ReservationFileService reservationFiles)
{
    ...
    public async Task<(LocalDocument Document, List<StoredSignature> Signatures)> StoreAsync(Reservation r, PdfTemplate? template, FillOfficialRegistrationCardRequest input, byte[] pdf, string? user, CancellationToken ct)
    {
        ...
        // Preserve PDF slots even when some people have not signed yet.
        var captured = new List<(StoredSignature Signature, int Position, bool Reused)>();
        var candidates = new List<(string Name, string? Id, string Role, string? Png, int Position)>
        {
            (input.PrimaryGuestName ?? r.Guest.FullName, input.PrimarySignerId ?? r.Guest.Id,
        ...
        foreach (var item in candidates)
        {
            ...
            var existing = await db.StoredSignatures.FirstOrDefaultAsync(x => x.HotelId == r.HotelId &&
                x.ConfirmationNumber == confirmation && x.ReservationId == reservationId && x.IsActive &&
                x.SignerKey == signature.SignerKey && x.SignerRole == signature.SignerRole &&
                x.SignatureHash == signature.SignatureHash, ct);
            if (existing is null) db.StoredSignatures.Add(signature);
            captured.Add((existing ?? signature, item.Position, existing is not null));
        }
        var signatures = captured.Select(x => x.Signature).ToList();
