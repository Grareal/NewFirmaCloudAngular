...
import { SignatureDraft, UdfCode } from './models';
import {
...
export class ApiService {
  ...
  getSignatureDraft(c: string) { return firstValueFrom(this.http.get<SignatureDraft>(`${B}/reservations/${encodeURIComponent(c)}/signature-draft`)); }
  saveSignatureDraft(c: string, revision: string | null, input: OfficialCardInput) {
    return firstValueFrom(this.http.put<SignatureDraft>(`${B}/reservations/${encodeURIComponent(c)}/signature-draft`, { revision, input }));
  }
  getUdfCodes() { return firstValueFrom(this.http.get<UdfCode[]>(`${B}/promotion-catalog/udf-values`)); }
